...
!!! tip "核心思路"
    ...
    维护一个**大小为 k 的小根堆**，堆中始终保存数组中最大的 k 个元素，堆顶就是第 k 大的。

    - 前 k 个元素直接入堆
    - 后续元素如果 `> 堆顶`，先入堆再弹出最小，保证堆大小始终为 k
    - 遍历完堆顶即为答案
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/top-k-frequent-elements/)

---

!!! tip "核心思路"

    `Counter` 统计频率后，用小根堆维护前 k 个高频元素。
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n log k) — 遍历统计 O(n)，堆操作 O(n log k)
    - **空间复杂度**：O(n) — 哈希表存所有元素频率

### LeetCode 295. 数据流的中位数

**难度**：⭐⭐⭐ 困难 &nbsp;|&nbsp; **标签**：`堆` `设计` `数据流`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/find-median-from-data-stream/)

---

!!! tip "核心思路"

    **对顶堆**：左半是大根堆（存较小的一半），右半是小根堆（存较大的一半），保持左堆比右堆多 0 或 1 个元素。

    - `addNum(x)`：
        - 如果 `x ≤ 左堆顶` → 入左堆；否则入右堆
        - 平衡：`左 > 右 + 1` → 左堆顶移到右堆；`右 > 左` → 右堆顶移到左堆
    - `findMedian()`：
        - 左堆元素多 → 左堆顶即为中位数
        - 两堆一样多 → 左堆顶 + 右堆顶 取平均
...
=== "Java"
    ...
    ```java
    class MedianFinder {
        private PriorityQueue<Integer> left;   // 大根堆
        private PriorityQueue<Integer> right;  // 小根堆
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(log n) — addNum 一次堆操作
    - **空间复杂度**：O(n) — 两个堆共存 n 个元素

### LeetCode 121. 买卖股票的最佳时机

**难度**：⭐ 简单 &nbsp;|&nbsp; **标签**：`动态规划` `数组`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock/)

---

!!! tip "核心思路"

    遍历时维护「前 i-1 天的最低价格」，第 i 天卖出能获得的最大利润 = `prices[i] - min_price`，取全局最大值。
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/jump-game/)

---

!!! tip "核心思路"

    贪心，遍历时维护**能跳到的最远位置** `max_pos`。

    - 如果 `i > max_pos`：当前格子都到不了，返回 `False`
    - 更新 `max_pos = max(max_pos, i + nums[i])`
    - 如果 `max_pos ≥ n - 1`：已能跳到终点，返回 `True`
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/jump-game-ii/)

---

!!! tip "核心思路"

    在「当前步」能覆盖的范围内，找出「下一步」能跳到的最远距离。

    - `pre_max`：当前步能到达的最远位置
    - `cur_max`：遍历到 `i` 时发现的能跳到的最远位置（用于下一步）
    - 当 `i == pre_max` 时，说明当前步的范围走完了，步数 +1，`pre_max = cur_max`
    - 一旦 `pre_max ≥ n - 1` 即可返回
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/partition-labels/)

---

!!! tip "核心思路"

    先记录每个字符**最后出现的位置**，然后贪心地扩展当前区间的右边界，当遍历下标 `r` 超过边界时，当前区间结束。

    - 逆序遍历记录 `last[c] = index`
    - 正序遍历，`cur` 维护当前区间所有字符的最后出现位置的最大值
    - 当 `r > cur` 时，区间结束，记录长度，重置左边界
    ...
    class Solution {
        public List<Integer> partitionLabels(String s) {
            ...
            for (int i = n - 1; i >= 0; i--) {
                ...
                if (last[idx] == 0) last[idx] = i;
            }

            List<Integer> res = new ArrayList<>();
            int l = 0, cur = 0;
            for (int r = 0; r < n; r++) {
                cur = Math.max(cur, last[s.charAt(r) - 'a']);
                if (r == cur) {
                    res.add(r - l + 1);
                    l = r + 1;
                }
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n) — 两次遍历
    - **空间复杂度**：O(1) — 哈希表/数组大小固定（26 个字母）

### LeetCode 70. 爬楼梯

**难度**：⭐ 简单 &nbsp;|&nbsp; **标签**：`动态规划` `数学`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/climbing-stairs/)

---

!!! tip "核心思路"

    入门 DP，转移方程 `f[i] = f[i-1] + f[i-2]`（第 i 阶 = 从 i-1 阶走 1 步 + 从 i-2 阶走 2 步）。
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n) — 一次遍历
    - **空间复杂度**：O(n) — 数组 f（可优化为 O(1)，只保留前两个）

### LeetCode 118. 杨辉三角

**难度**：⭐ 简单 &nbsp;|&nbsp; **标签**：`动态规划` `数组`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/pascals-triangle/)

---

!!! tip "核心思路"

    一行一行递推：`triangle[i][j] = triangle[i-1][j-1] + triangle[i-1][j]`，越界补 0。
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n²) — 一共生成 n 行，第 i 行 i 个元素
    - **空间复杂度**：O(n²) — 结果数组
...
!!! tip "核心思路"
    ...
    每天两个状态：**偷** 或 **不偷**，构成简单状态机。

    - `f[i][0]`：第 i 天**不偷** = `max(f[i-1][0], f[i-1][1])`
    - `f[i][1]`：第 i 天**偷** = `f[i-1][0] + nums[i]`（前一天必须没偷）
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n) — 一次遍历
    - **空间复杂度**：O(n) — DP 数组（可优化为 O(1)，只需前一天的偷/不偷值）

### LeetCode 279. 完全平方数

**难度**：⭐⭐ 中等 &nbsp;|&nbsp; **标签**：`动态规划` `完全背包` `数学`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/perfect-squares/)
...
!!! tip "核心思路"
    ...
    !!! tip "状态定义"
        ...
        `f[j]` = 装满容量为 `j` 的背包所需的最少完全平方数个数。

        - 初始化 `f[0] = 0`，其余为 `+∞`
        - 完全背包内层循环从 `v` 到 `n`（**正序**，因为物品可重复用）
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n × √n) — √n 个物品，遍历 n 个容量
    - **空间复杂度**：O(n) — DP 数组

### LeetCode 322. 零钱兑换

**难度**：⭐⭐ 中等 &nbsp;|&nbsp; **标签**：`动态规划` `完全背包` `数组`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/coin-change/)

---

!!! tip "核心思路"

    和 LeetCode 279 完全平方数一模一样 —— **完全背包求最小物品数**。

    - **物品**：`coins`（可无限次使用）
    - **背包容量**：`amount`
    - **目标**：恰好装满的最小硬币数
    - 最后 `f[amount]` 仍是 `INF` → 返回 `-1`（凑不出来）
...
**难度**：⭐⭐ 中等 &nbsp;|&nbsp; **标签**：`字典树` `记忆化搜索` `哈希表` `字符串` `动态规划`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/word-break/)

---

!!! tip "核心思路"

    经典的线性 DP，`f[i]` 代表 `前 i 个字符` 可以被拼出来，`f[i] |= f[j] and s[j + 1, i] in dic`。

    - **状态定义**：`f[i]` = 字符串前 `i` 个字符能否被字典拼出
    - **转移方程**：`f[i] = f[j] && s[j+1:i+1] in wordDict`，枚举分割点 `j`
    - **初始化**：`f[0] = True`（空串合法）
    - **字符串预处理**：在 `s` 前补一个空格，下标统一从 `1` 开始
...
=== "Java"
    ...
    class Solution {
        public boolean wordBreak(String s, List<String> wordDict) {
            ...
            // 在 s 前补一个空格，下标统一从 1 开始
            s = " " + s;
            boolean[] f = new boolean[n + 1];
            f[0] = true;
            for (int i = 1; i <= n; i++) {
                for (int j = 0; j < i; j++) {
                    if (f[j] && dic.contains(s.substring(j + 1, i + 1))) {
                        f[i] = true;
                        break;
                    }
                }
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n² × L) — n² 个状态转移，每次比较字符串长度 L
    - **空间复杂度**：O(n) — DP 数组 + 字典哈希表

### LeetCode 300. 最长递增子序列

**难度**：⭐⭐ 中等 &nbsp;|&nbsp; **标签**：`二分查找` `动态规划` `数组`
...
!!! tip "核心思路"
    ...
    **贪心 + 二分查找（O(n log n)）**：
    ...
    - 维护数组 `lens`，`lens[i]` 表示长度为 `i+1` 的上升子序列的**最小结尾值**
    - 遍历每个数 `num`，在 `lens` 中二分查找第一个 `>= num` 的位置
        - 若找到，替换该位置的值为 `num`（更小的结尾值更有利于后续拼接）
        - 若没找到（`num` 比所有元素都大），追加到末尾（长度 +1）
    - 最终 `len(lens)` 即为最长递增子序列的长度
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n log n) — 遍历 n 个数，每个数做一次二分查找
    - **空间复杂度**：O(n) — lens 数组最多存储 n 个数
...
!!! tip "核心思路"
    ...
    定义 `f[i][0]`、`f[i][1]` 分别为以 `i` 结尾的乘积**最小**、**最大**的子数组值。

    由于存在负数，最小的乘积乘上负数可能变成最大，反之亦然，因此需要同时维护 `max` 和 `min`。

    - 当 `nums[i] > 0`：
        - `f[i][1] = max(f[i-1][1] * nums[i], nums[i])`
        - `f[i][0] = min(f[i-1][0] * nums[i], nums[i])`
    - 当 `nums[i] < 0`：
        - `f[i][1] = max(nums[i], f[i-1][0] * nums[i])`（负负得正）
        - `f[i][0] = min(nums[i], f[i-1][1] * nums[i])`（正负得负）
...
=== "Python"
    ...
    class Solution:
        def maxProduct(self, nums: List[int]) -> int:
            n = len(nums)
            # f[i][0] = 以 i 结尾的子数组乘积的最小值
            # f[i][1] = 以 i 结尾的子数组乘积的最大值
            f = [[x, x] for x in nums]
            res = f[0][1]
...
=== "Java"
    ...
    class Solution {
        public int maxProduct(int[] nums) {
            ...
            for (int i = 1; i < n; i++) {
                if (nums[i] > 0) {
                    f[i][0] = Math.min(f[i - 1][0] * nums[i], nums[i]);
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n) — 一次遍历
    - **空间复杂度**：O(n) — DP 数组（可优化为 O(1)，只需前一天的 min/max）

### LeetCode 416. 分割等和子集

**难度**：⭐⭐ 中等 &nbsp;|&nbsp; **标签**：`动态规划` `01背包` `数组`
...
!!! tip "核心思路"
    ...
    建模为 **01 背包问题**：能否选出一些数，使其总和恰好为 `target = sum(nums) / 2`。

    - **目标**：`sum % 2 != 0` → 直接返回 `False`
    - **状态**：`f[i]` = 能否凑出总和 `i`
    - **转移**：`f[i] = f[i] or f[i - num]`（选或不选当前数）
    - **遍历顺序**：内层循环**倒序**（01 背包特性，每个数只能用一次）
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n × target) — n 个数，背包容量 target
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/longest-valid-parentheses/)

---

!!! tip "核心思路"

    DP，`f[i]` 表示以 `s[i]` 结尾的最长有效括号子串长度。只关心 `s[i] == ')'` 的情况。

    - **`s[i-1] == '('`**：配对，`f[i] = f[i-2] + 2`
    - **`s[i-1] == ')'`**：需要向前找匹配的左括号，位置为 `pre = i - f[i-1] - 1`
        - 若 `s[pre] == '('`，则 `f[i] = f[i-1] + 2 + f[pre-1]`（加上前面的有效长度）
    ...
    class Solution:
        def longestValidParentheses(self, s: str) -> int:
            ...
            for i in range(1, n):
                if s[i] == ')':
                    if s[i - 1] == '(':
                        f[i] = (f[i - 2] if i - 2 >= 0 else 0) + 2
                    else:
                        pre = i - f[i - 1] - 1
                        if pre >= 0 and s[pre] == '(':
                            f[i] = f[i - 1] + 2 + (f[pre - 1] if pre - 1 >= 0 else 0)
...
**难度**：⭐⭐ 中等 &nbsp;|&nbsp; **标签**：`动态规划` `组合数学` `数组`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/unique-paths/)

---

!!! tip "核心思路"

    简单二维 DP，`f[i][j]` 表示到达 `(i, j)` 的不同路径数。

    - `f[0][0] = 1`（起点）
    - `f[i][j] = f[i-1][j] + f[i][j-1]`（只能向下或向右走）
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n × m) — 遍历整个网格
    - **空间复杂度**：O(n × m) — DP 数组（可优化为 O(m)，仅保留上一行）

### LeetCode 64. 最小路径和

**难度**：⭐⭐ 中等 &nbsp;|&nbsp; **标签**：`动态规划` `数组` `矩阵`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/minimum-path-sum/)

---

!!! tip "核心思路"

    和 62 相同模板，只是求和变为取最小值：`f[i][j] = min(f[i-1][j], f[i][j-1]) + grid[i][j]`。
...
??? info "复杂度分析"
    ...
    - **时间复杂度**：O(n × m) — 遍历整个网格
    - **空间复杂度**：O(n × m) — DP 数组（可优化 O(m)）

### LeetCode 5. 最长回文子串

**难度**：⭐⭐ 中等 &nbsp;|&nbsp; **标签**：`双指针` `动态规划` `字符串`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/longest-palindromic-substring/)

---

!!! tip "核心思路"

    **中心扩展法**：分别枚举所有**奇数**和**偶数**回文串的中心，向两边扩展找到最长回文。

    - 奇数：中心为 `i`，左右指针从 `i-1`、`i+1` 开始
    - 偶数：中心为 `(i, i+1)`，左右指针从 `i`、`i+1` 开始
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/longest-common-subsequence/)

---

!!! tip "核心思路"

    经典二维 DP，`f[i][j]` = `text1[1..i]` 和 `text2[1..j]` 的最长公共子序列长度。
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/edit-distance/)

---

!!! tip "核心思路"

    二维 DP，`f[i][j]` 表示将 `word1[1..i]` 转换成 `word2[1..j]` 的最少操作步数。

    - **初态**：`f[0][j] = j`（从空串插入 j 次），`f[i][0] = i`（删除 i 次）
    - **转移**（分别对应三种操作）：
        - `word1[i] == word2[j]`：`f[i][j] = f[i-1][j-1]`（无需操作）
        - 否则：`f[i][j] = min(f[i-1][j], f[i][j-1], f[i-1][j-1]) + 1`
            - `f[i-1][j]` → 删除 `word1[i]`
            - `f[i][j-1]` → 插入 `word2[j]`
            - `f[i-1][j-1]` → 替换 `word1[i]` 为 `word2[j]`
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/single-number/)

---

!!! tip "核心思路"

    利用异或性质：`a ^ a = 0, a ^ 0 = a`，且异或满足交换律。全部异或一遍，成对的数抵消，剩下的就是答案。
...
!!! tip "核心思路"
    ...
    **摩尔投票法（一换一）**：不同元素两两抵消，最后剩下的就是多数元素。

    - `times == 0` 时更新候选，`times = 1`
    - 相同则 `times++`，不同则 `times--`
    - 最终 `cur` 即为答案
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/sort-colors/)

---

!!! tip "核心思路"

    **荷兰国旗问题**（三指针）—— 一次遍历原地排序，`0` 放左边、`2` 放右边、`1` 自然居中。

    - `l`：0 的右边界，`l` 左侧全是 0
    - `r`：2 的左边界，`r` 右侧全是 2
    - `i`：当前遍历指针
    - `nums[i] == 0` → 与 `nums[l]` 交换，`l++`、`i++`
    - `nums[i] == 2` → 与 `nums[r]` 交换，`r--`（`i` 不动，因为换过来的可能是 0）
    - `nums[i] == 1` → `i++`（1 留在中间）
...
[🔗 LeetCode 原题链接](https://leetcode.cn/problems/next-permutation/)
...
!!! tip "核心思路"
    ...
    1. **从后往前**找第一个非递增的数 `nums[i]`（`nums[i] < nums[i+1]`）
    2. **从后往前**找第一个比 `nums[i]` 大的数 `nums[j]`，交换
    3. **反转** `nums[i+1:]`（原来递减 → 变为递增，得到最小排列）

    - 若第一步找不到（整个数组递减），直接反转整个数组
...
**难度**：⭐⭐ 中等 &nbsp;|&nbsp; **标签**：`数组` `双指针` `二分查找`

[🔗 LeetCode 原题链接](https://leetcode.cn/problems/find-the-duplicate-number/)

---

!!! tip "核心思路"

    将数组视为链表：`i → nums[i]`，因为有重复数，则必定存在**环**。用 **Floyd 判环法**（快慢指针）找环入口，环入口即为重复数。

    - **Phase 1**：快慢指针从 `0` 出发，`slow = nums[slow]`、`fast = nums[nums[fast]]`，相遇时停下
    - **Phase 2**：头指针从 `0` 出发，与 slow 同步前进，相遇点即为环入口（重复数）
