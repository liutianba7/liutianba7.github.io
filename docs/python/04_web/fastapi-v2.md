...
FastAPI 是一个**现代、高性能**的 Python Web 框架，专用于构建 **API 服务**。它的核心公式很简单：

!!! tip ""
    **FastAPI = Starlette（异步网络能力） + Pydantic（数据校验） + 类型注解驱动**

- **Starlette** 提供底层的 ASGI 网络通信能力，赋予 FastAPI 高性能和异步支持
- **Pydantic** 提供数据校验和序列化能力，确保数据安全可靠
- **类型注解**是连接一切的"胶水"——通过 Python 原生的类型提示，框架自动推导出校验规则、API 文档、序列化行为

这使得 FastAPI 成为**编写 API 效率极高**的框架：你只需要写 Python 代码，文档、校验、序列化全都自动完成。

!!! quote "FastAPI 的设计哲学"
    FastAPI 的设计目标不是成为一个"全栈框架"，而是在 **API 开发**这个细分领域做到极致。它不提供模板引擎、不提供 ORM、不提供 Admin 后台——但它让你**自由选择**最适合的工具来组合。

### 0.2 架构全景：一次请求的生命周期

理解 FastAPI 最好的方式，是看一个请求从进入到返回，经过了哪些环节：
...
┌─────────────────────────────────────────────────┐
│            ASGI Server (Uvicorn)                 │
│  接收 HTTP 请求，将其转为 ASGI 事件               │
└─────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────┐
│              Middleware 链（洋葱模型）              │
│  ️ 请求前处理：CORS 检查、日志、认证              │
│  ️ 请求后处理：添加响应头、压缩、日志              │
│  拦截：不调用 call_next 则直接返回                 │
└─────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────┐
│              路由匹配（Router）                    │
│  根据 URL Path + HTTP Method 找到对应的处理函数     │
│  支持路径参数提取（如 /users/{id} → id=123）       │
└─────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────┐
│           依赖注入（Dependency Injection）         │
│  自动执行 Depends 链，收集所有依赖：               │
│  数据库 Session、认证信息、公共参数...              │
└─────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────┐
...
│  路径参数 → int 校验                             │
│  查询参数 → Query() 校验                         │
│  请求体 → Pydantic Model 校验                    │
│  校验失败 → 自动返回 422 错误                     │
└─────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────┐
│          路径操作函数执行（Your Business Logic）   │
│  你写的 async def 在此执行                        │
└─────────────────────────────────────────────────┘
    │
    ▼
...
#### 2.10.1 为什么需要 APIRouter

随着项目规模增长，把所有路由写在 `main.py` 里会变得**不可维护**。`APIRouter` 是 FastAPI 提供的**模块化路由工具**，让你按业务模块拆分路由文件。

!!! tip "核心思想"
    **`APIRouter` 就像是"小号的 FastAPP"**——它支持所有 `app.get()`、`app.post()` 的操作，但最终通过 `app.include_router()` 挂载到主应用。

| 问题 | APIRouter 的解决方案 |
|------|-------------------|
| 所有路由挤在一个文件 | 按模块拆分到独立文件 |
| URL 前缀重复编写 | 统一通过 `prefix` 参数配置 |
| 模块级权限/中间件 | 通过 `dependencies` 参数统一配置 |
| 路由难以复用 | 可独立导入和测试 |

#### 2.10.2 基本用法

创建路由模块，使用 `@router` 装饰器替代 `@app`：
...
#### 2.10.3 include_router 参数详解

`app.include_router()` 接受多个参数，用于统一配置路由模块：

```python
app.include_router(
    books_router,
    prefix="/books",          # 所有路由自动添加前缀
    tags=["图书"],             # 自动文档分组标签
    dependencies=[Depends(verify_token)],  # 模块级依赖
    responses={404: {"description": "Not found"}},  # 模块级响应定义
...
| 参数 | 作用 | 示例 |
|------|------|------|
| `prefix` | URL 前缀，所有路由前面自动拼接 | `/books` → 最终路径 `/books/` |
| `tags` | 自动文档中的分组标签 | `["图书"]` |
| `dependencies` | 模块级依赖，所有路由生效 | `[Depends(verify_token)]` |
| `responses` | 公共响应定义 | `{404: {"description": "Not found"}}` |
| `default_response_class` | 默认响应类型 | `ORJSONResponse`（JSON 性能优化） |

!!! note "prefix 对路由路径的影响"
    router 中定义的路由是**相对路径**，挂载时 `prefix` 会自动拼接：

    ```python
    # router 中定义
    @router.get("/{book_id}")      # 路径是 /{book_id}
    async def get_book(book_id: int):
        ...

    # app.include_router(router, prefix="/books")
    # 最终访问路径: /books/{book_id}
    ```

    `<router_path>` + `<prefix_path>` 会合并为最终路径。

#### 2.10.4 多模块组织与汇总

当有多个业务模块时，推荐用**汇总路由器**统一管理：
...
!!! tip "推荐的模块组织方式"
    ...
    开发新模块时只需三步：
    1. 在 `endpoints/` 下新建文件，定义 `router`
    2. 在 `api.py` 中 `include_router`
    3. 完成

    不需要修改 `main.py`，符合**开闭原则**。

#### 2.10.5 路由嵌套 (include_router 链)

`APIRouter` 可以嵌套挂载，形成**多级路由结构**：
...
!!! note "路径拼接规则"
    ...
    最终: /api/v1/items/{item_id}
    ```

#### 2.10.6 模块级依赖（鉴权实战）

`APIRouter` 的 `dependencies` 参数是实现**模块级鉴权**的最佳方式：
...
admin_router = APIRouter(
    prefix="/admin",
    tags=["管理后台"],
    dependencies=[Depends(verify_admin_user)],  # 模块级
)
...
!!! tip "依赖作用范围对比"
    ...
    | **模块级** | `APIRouter(dependencies=[...])` | 该模块所有路由 |
    | **路由级** | `@router.get("/", dependencies=[...])` | 单个路由 |
    | **参数级** | `func(param: Annotated[...] = Depends(...))` | 单个函数参数 |

    遵循**最小范围原则**：能用参数级就别用全局，降低耦合度。
...
#### 7.0.1 SQLAlchemy = Core + ORM

!!! tip "SQLAlchemy 不是单纯的 ORM"
    SQLAlchemy 实际上是**两个库合二为一**：

    | 层次 | 名称 | 类比 | 作用 |
    |------|------|------|------|
    | **Core** | 数据库抽象层 | 高级版 `sqlite3` / `pymysql` | 提供统一的 SQL 构造和执行接口 |
    | **ORM** | 对象关系映射 | 让你用 Python 对象操作数据库表 | 建立在 Core 之上，提供更高层的抽象 |

    两层可以**独立使用**，也可以**混合使用**：

    ```python
    # 只用 Core：写 SQL 一样的代码，但跨数据库兼容
    ...
    本笔记聚焦 **ORM 层**（这也是 90% 场景用的方式），但理解 Core 的存在对后续理解执行机制很重要。
...
**驱动（Driver）** 是实现了 Python DB-API 2.0 规范（[PEP 249](https://peps.python.org/pep-0249/)）的 Python 包，负责与特定数据库进行**网络通信**。

```python
# 驱动的本质：把 Python 代码转成数据库协议
# pymysql 内部大致做的事情：
import socket

# 1. 建立 TCP 连接到 MySQL 3306 端口
conn = socket.create_connection(("localhost", 3306))

# 2. 用 MySQL 协议发送 SQL 语句
conn.send(b"SELECT * FROM users WHERE id = 1")

# 3. 解析 MySQL 协议返回的数据
result = conn.recv(8192)
# 然后转换成 Python 对象（元组、字典等）
```

| 数据库 | 同步驱动 | 异步驱动 | 驱动相当于 |
|--------|---------|---------|-----------|
| MySQL | `pymysql` | `aiomysql` | **翻译官**：把 Python 的 SQL 请求翻译成 MySQL 能理解的协议 |
| PostgreSQL | `psycopg2` | `asyncpg` | **信使**：建立 TCP 连接，发送 SQL，带回结果 |
| SQLite | `sqlite3`（标准库自带） | `aiosqlite` | **文件操作员**：直接读写本地数据库文件 |

!!! tip "同步 vs 异步驱动"
    - **同步驱动**：发 SQL → 等待结果 → 线程阻塞，期间不能做其他事
    - **异步驱动**：发 SQL → await → 事件循环可以处理其他请求 → 结果返回后继续执行

    这就是 FastAPI + SQLAlchemy 异步组合的性能优势来源。

#### 7.0.4 为什么需要 `sqlalchemy[asyncio]`

在 7.1 安装中你会看到 `pip install "sqlalchemy[asyncio]"`，这个 `[asyncio]` 额外安装了什么？

```
sqlalchemy[asyncio]
    ├── sqlalchemy              # 核心库（同步功能）
    └── greenlet                # 异步支持的关键依赖
        ↑ greenlet 让 SQLAlchemy 在异步环境中能安全地管理数据库连接和会话，
          避免同步代码阻塞事件循环。
```

!!! warning "同步驱动 ≠ 异步能用的驱动"
    安装 `sqlalchemy[asyncio]` 只是让 SQLAlchemy 支持了 `async/await` 语法。
    但真正"异步地"执行 SQL，还需要**异步驱动**（如 `aiomysql`、`asyncpg`），
    否则只是"用 async 语法调用了同步驱动"，性能没有提升。
...
!!! tip "一句话理解 Session"
    **Session 不是数据库连接，是你的"工作区（Workspace）"。**
    ...
    你在 Session 中做的所有操作（增删改查），都先缓存在这个工作区里，只有你调 `commit()` 时，才会一次性同步到数据库。

```python
# Session 的工作方式
async with AsyncSessionLocal() as session:
    # ① 在"工作区"中操作——此时数据库毫无变化
    book = Book(title="Python 入门")
    session.add(book)          # 只是标记"待保存"

    # ② 查询也会经过 Session 的缓存
    stmt = select(Book).where(Book.id == 1)
    result = await session.execute(stmt)  # 先去数据库查
...
| 职责               | 说明                                       | 类比                    |
| ---------------- | ---------------------------------------- | --------------------- |
| **事务管理**         | Session 开启时自动开始事务，commit/rollback 控制事务边界 | 就像 Git 的 staging area |
| **Identity Map** | 一个主键只对应一个 Python 对象，多次查询返回同一对象           | 类似字典缓存，避免数据不一致        |
| **脏追踪**          | 自动记录哪些对象被修改了，commit 时只提交变更的部分            | 类似 Git 的 diff         |
...
!!! note "Session 的生命周期"
    ```
    创建 Session ─→ 开始事务 ─→ 执行操作 ─→ commit / rollback ─→ 关闭 Session
                                                                    │
                                               Connection 释放回连接池
    ```

    在 FastAPI 中，通过 `yield` 依赖管理这个生命周期：每个请求创建一个 Session，响应返回后自动关闭。
...
class User(Base):  
	...
	roles: Mapped[list["Role"]] = relationship(  
		secondary=user_role, # 告诉 sqlAlchemy，关联关系是这张中间表说明的
		back_populates="users",  
		lazy="selectin" # 推荐：避免 N+1  
	)
...
> 完整用法详见 [2.10 路由分发 (APIRouter)](#210-路由分发-apirouter)

在项目工程中，所有路由模块通过 `app.include_router()` 汇总挂载到主应用。具体组织方式可以参考 [10.1 项目架构](#101-项目架构) 中的目录结构。
