...
<p>
	<img src='../../assets/imgs/python/nlp/nlp02_rnn结构图.png'>
</p>
 ...
 **计算损失 (Loss Calculation)**：将每个时刻的预测值 $y_t$ 与真实标签（Ground Truth）进行对比，计算总损失 $Loss = \sum L_t$。
...
RNN 可以从**两个维度**进行分类：**输入输出结构** 和 **内部结构**。

##### 按输入输出结构分类

根据输入和输出序列长度关系，可分为四种典型模式：

| 类型                       | 输入长度 | 输出长度 | 应用场景                   | 示例                |
| ------------------------ | ---- | ---- | ---------------------- | ----------------- |
| **N → N (Many to Many)** | N    | N    | 每个输入对应一个输出             | 中文分词、词性标注、命名实体识别  |
| **N → 1 (Many to One)**  | N    | 1    | 整个序列输出一个结果             | 文本分类、情感分析、垃圾邮件分类  |
| **1 → N (One to Many)**  | 1    | N    | 单个输入（每次输入都是同一个值）输出一个序列 | 图像生成文字描述、根据类别生成文本 |
| **N → M (Many to Many)** | N    | M    | 输入输出长度不同               | 机器翻译、文本摘要、对话生成    |

##### 按内部结构分类

| 类型                  | 特点                                    |
| ------------------- | ------------------------------------- |
| **基础 RNN**          | 最简单的循环结构，容易梯度消失                       |
| **LSTM**            | 引入门控机制（输入门、遗忘门、输出门）和细胞状态，解决长序列梯度消失    |
| **GRU**             | LSTM 的简化版，将遗忘门和输入门合并为更新门，参数更少训练更快     |
| **双向 RNN (Bi-RNN)** | 同时利用**过去和未来**的信息，两层 RNN 分别从左到右和从右到左扫描 |
| **单层 / 多层堆叠**       | 多层 RNN 堆叠可以提取更高层次的语义特征                |

常见组合：**BiLSTM**、**BiGRU** 是 NLP 任务中非常常用的基础模块。
...
PyTorch 中 `nn.LSTM` 的关键初始化参数：

| **参数**          | **说明**                  |
| --------------- | ----------------------- |
| `input_size`    | 输入特征的数量（例如词向量的维度）。      |
| `hidden_size`   | 隐状态（Hidden State）的特征数量。 |
| `num_layers`    | LSTM 的层数（堆叠 LSTM）。默认为 1。  |
| `bidirectional` | 是否为双向 LSTM。默认为 `False`。  |
| `batch_first`   | 输入输出维度顺序。`True` 为 `(batch, seq, feature)`，`False` 为 `(seq, batch, feature)`。 |
| `dropout`       | 除了最后一层，每层输出后添加 Dropout。默认为 0。 |
...
# 参数说明：input_size=10, hidden_size=20, num_layers=1
lstm = nn.LSTM(10, 20, 1, batch_first=True)
...
PyTorch 中 `nn.GRU` 的关键初始化参数，和 RNN/LSTM 保持一致：

| **参数**          | **说明**                  |
| --------------- | ----------------------- |
| `input_size`    | 输入特征的数量（例如词向量的维度）。      |
| `hidden_size`   | 隐状态（Hidden State）的特征数量。 |
| `num_layers`    | GRU 的层数（堆叠 GRU）。默认为 1。  |
