...
其中 $x_1, x_2$ 是输入信号，$y$ 是输出信号，$w_1, w_2$ 是**权重**，$b$ 是**偏置**。

!!! tip "权重与偏置的作用"
    - **权重**：控制各个信号的重要性，权重越大，对应信号的重要性越高
    - **偏置**：控制神经元被激活的容易程度

##### 2.1.3 引入激活函数

感知机的计算包含两步：先计算输入信号的加权总和，再根据阈值决定输出。定义一个**激活函数** $h(x)$：
...
$h(x)$ 将输入信号的加权总和转换为输出信号，起到"激活神经元"的作用，因此被称为**激活函数**。

#### 2.2 激活函数

> 激活函数是连接感知机和神经网络的桥梁。如果没有激活函数，整个神经网络等效于单层线性变换。**激活函数必须是非线性函数**，为神经网络引入非线性能力。

##### 2.2.1 阶跃函数（Binary Step）

最简单的激活函数，为输入设置一个"阈值"，超过阈值则切换输出。

| 项目 | 内容 |
|------|------|
| 公式 | $f(x) = \begin{cases} 0, & x < 0 \\ 1, & x \geq 0 \end{cases}$ |
| 导数 | $f'(x) = 0$ |
| 特点 | 简单直观，但导数为 0，无法用于梯度下降 |
...
> 参考 [Sigmoid 维基百科](https://en.wikipedia.org/wiki/Sigmoid_function)

| 项目 | 内容 |
|------|------|
| 公式 | $\displaystyle f(x) = \frac{1}{1 + e^{-x}}$ |
| 导数 | $f'(x) = f(x)(1 - f(x))$ |
| 值域 | $(0, 1)$ |
| 用途 | 二分类输出层 |

!!! warning "Sigmoid 的局限性"
    - 输入在 $[-6, 6]$ 之外时，输出变化很小，导数接近 0，导致**梯度消失**
    - 输出**不以 0 为中心**（均大于 0），可能影响后续梯度更新方向
    - 导数范围 $(0, 0.25)$，梯度较小，逐层反向传播时梯度会呈指数级衰减
...
| 项目 | 内容 |
|------|------|
| 公式 | $\displaystyle f(x) = \frac{1 - e^{-2x}}{1 + e^{-2x}}$ |
| 导数 | $f'(x) = 1 - f(x)^2$ |
| 值域 | $(-1, 1)$ |
| 特点 | 关于原点中心对称，输出均值为 0 |

!!! tip "Tanh vs Sigmoid"
    Tanh 输出以 0 为中心，梯度范围更大 $(0, 1]$，收敛速度相对更快，但仍存在梯度消失现象。适合**浅层网络的隐藏层**。
...
##### 2.2.4 ReLU 函数（修正线性单元）

> 参考 [ReLU 论文](https://www.cs.toronto.edu/~hinton/absps/reluICML.pdf)

| 项目 | 内容 |
|------|------|
| 公式 | $f(x) = \max(0, x) = \begin{cases} 0, & x \leq 0 \\ x, & x > 0 \end{cases}$ |
...
!!! success "ReLU 的优势"
    - 计算简单，收敛速度快
    - 不存在梯度消失（正区间导数为 1）
    - 具有**稀疏性**：部分神经元输出为 0，减少计算量

!!! warning "神经元死亡问题"
    当神经元的输入持续为负数时，ReLU 输出始终为 0，导致神经元永不被激活。可使用 **Leaky ReLU** 或 **PReLU** 解决。
...
| 项目 | 内容 |
|------|------|
| 公式 | $\displaystyle y_k = \frac{e^{x_k}}{\sum_{i=1}^{n} e^{x_i}}, \quad k = 1 \sim n$ |
| 导数 | $\displaystyle \frac{\partial y_k}{\partial x_i} = \begin{cases} y_k(1 - y_k), & k = i \\ -y_k y_i, & k \neq i \end{cases}$ |
| 值域 | $(0, 1)$，且总和为 1 |
| 用途 | **多分类**输出层 |

!!! info "Softmax 的特点"
    - 将任意实数向量转换为**概率分布**，输出总和为 1
    - 是 Sigmoid 在多分类上的推广
    - 放大输入中的较大值，压缩较小值，起到类别区分作用

    例如输入 `[1, 3, 5, 10]` 经 Softmax 后输出接近 `[0.0001, 0.0009, 0.0067, 0.9923]`
...
| 名称 | 公式 | 特点 |
|------|------|------|
| **Identity** | $f(x) = x$ | 回归输出层默认 |
| **Leaky ReLU** | $f(x) = \begin{cases} \alpha x, & x \leq 0 \\ x, & x > 0 \end{cases}$ | 在负数区域引入小斜率 $\alpha$（如 0.01），解决神经元死亡 |
| **PReLU** | $f(x) = \begin{cases} \alpha x, & x \leq 0 \\ x, & x > 0 \end{cases}$ | $\alpha$ 为**可训练参数**，而非固定常数 |
| **RReLU** | $f(x) = \begin{cases} \alpha x, & x \leq 0 \\ x, & x > 0 \end{cases}$ | $\alpha$ 在训练时从均匀分布中随机采样 |
| **ELU** | $f(x) = \begin{cases} \alpha(e^x - 1), & x \leq 0 \\ x, & x > 0 \end{cases}$ | 负数区域平滑趋向 $-\alpha$，均值更接近 0 |
| **Swish / SiLU** | $f(x) = \dfrac{x}{1 + e^{-x}}$ | Google 提出，无上界有下界、平滑、非单调 |
| **Softplus** | $f(x) = \ln(1 + e^x)$ | ReLU 的平滑近似，导数恰为 Sigmoid |
...
|--------|----------|------|
| ⭐首选 | **ReLU** | 默认首选，简单高效 |
| ⭐备选 | **Leaky ReLU / PReLU** | ReLU 效果不好时尝试 |
| ❌避免 | Sigmoid | 易导致梯度消失 |
| ⚠️慎用 | Tanh | 输出均值为 0，适合浅层网络 |
...
使用 Digit Recognizer 数据集（Kaggle）进行手写数字识别。

**数据集信息：**

- 手绘数字 0~9 的灰度图像，每张 $28 \times 28 = 784$ 像素
- 每个像素取值 $0 \sim 255$ 表示亮度
- 第 1 列为标签，后 784 列为像素值
...
def init_network():
    return joblib.load("../data/nn_sample")
...
print(f"Accuracy: {accuracy_cnt / len(x):.4f}")
```

!!! info "批处理推理"
    使用 **批处理**（batch）可以充分利用矩阵运算的并行性，大幅提升推理效率。`np.argmax(y_batch, axis=1)` 取出每个样本预测概率最大的类别索引。
