...
- **原子性**（`Atomicity`）：事务是最小的执行单位，不允许分割。事务内的操作要么全部执行成功，要么读不执行。
- **一致性**（`Consistency`）：执行事务前后，数据保持一致，例如转账业务中，无论事务是否成功，转账者和收款人的总额应该是不变的；
- **隔离性**（`Isolation`）：并发执行的各个事务是相互独立的，互不干扰的！
...
- 开启**慢查询日志**，用 `mysqldumpslow` 分析高频慢 SQL
- 用 `EXPLAIN` 查看执行计划，重点关注 `type`（是否全表扫描）和 `Extra`（是否 Using filesort）

**2. 索引、表结构和 SQL 优化**

- **加索引**：遵循最左前缀原则，覆盖索引减少回表，避免索引失效场景
- **表结构**：字段尽量 `NOT NULL`，避免冗余列，大字段拆分
- **SQL 写法**：只 `SELECT` 需要的列，避免 `SELECT *`，`IN` 替换 `OR`
